...
- git
- Julia 1.12.6 by default, matching `distributed-queue/Manifest.toml`
...
If you add Julia packages, rebuild the AMI by rerunning `prepare-base-ami.sh` on a temporary instance and creating a new AMI. Leave the script default Julia version aligned with `distributed-queue/Manifest.toml`; it is currently Julia 1.12.6.
